...
export default function CustomersTable({dataList}: BasicTableProps){
    ...
    return <Box className={cssModule.box} sx={{ width: '100%' }}>
    <Typography className={cssModule.tableTitle}
          sx={{ flex: '1 1 100%' }}          
          id="tableTitle"
          component="p"
        >
    ...
    <Table sx={{ minWidth: 511, maxWidth: 1280 }} aria-label="simple table">
        ...
        <TableBody>
          {dataList && dataList.map((row) => {
            const imgSrc = row.image || row.photo || '/images/default-avatar.png';
            return (<TableRow
              key={row.name}
              sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
            >
              <TableCell component="th" scope="row"><div className={cssModule.photoAndName}><Image src={imgSrc} width={36} height={36} alt='portrait photo'/>
                {row.name}</div>
              </TableCell>
